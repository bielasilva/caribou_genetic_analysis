...
## Import files
```{r import, cache=T}
labels.df <- read.csv("/mmfs1/scratch/CaribouProject/labels.csv")[1:121,-1]
```

## Plot Best K
```{r best-k, fig.height = 5, fig.width = 20, fig.align = "center", cache=T}

K1File <- "/mmfs1/scratch/CaribouProject/run/PopulationGenetics/Admixture/logs/Admixture.ntg_K-1012434.out"
K2File <- "/mmfs1/scratch/CaribouProject/run/PopulationGenetics/Admixture/logs/Admixture.ntg_K-1012435.out"
K3File <- "/mmfs1/scratch/CaribouProject/run/PopulationGenetics/Admixture/logs/Admixture.ntg_K-1012436.out"
K4File <- "/mmfs1/scratch/CaribouProject/run/PopulationGenetics/Admixture/logs/Admixture.ntg_K-1012437.out"
K5File <- "/mmfs1/scratch/CaribouProject/run/PopulationGenetics/Admixture/logs/Admixture.ntg_K-1012438.out"
...
for (k in seq(1, 5)) {
  k.table <- read.table(paste0("Caribou.ntg.", k, ".Q"))
...
basePath <- "/mmfs1/scratch/CaribouProject/analysis/admixture/ntg/"

for (k in seq(1, 5)) {
data.df <- cbind(labels.df, read.table(paste0(basePath,"Caribou.ntg.", k, ".Q"))) %>%  
...
plt <- data.df %>%
  ...
  facet_grid(~factor(Pop, levels=c("West","Central","East")), scale = "free_x", space = "free_x") +
  theme_light() +
  ...
  labs(title=paste0("Admixture No Togiak | K=",k),
